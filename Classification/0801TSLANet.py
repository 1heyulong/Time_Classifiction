...
class model_training(L.LightningModule):
    def __init__(self):
        ...
        self.criterion = FocalLoss()  # 可以替换为 FocalLoss 或其他损失函数
    ...
    def configure_optimizers(self):
        # 使用 SGD 优化器（也可以换成 AdamW）
        optimizer = optim.SGD(
            self.parameters(),
            lr=args.train_lr,          # 初始学习率
            momentum=0.9,              # 动量
        ...
        total_steps = steps_per_epoch * args.num_epochs
        warmup_steps = int(0.1 * total_steps)  # 预热 10% 的训练步骤
...
if __name__ == '__main__':
    ...
    CHECKPOINT_PATH = f"/tf_logs/0801_store_result/{args.name}"
    ...
    text_save_dir = f"/tf_logs/Classification/0801textFiles"
