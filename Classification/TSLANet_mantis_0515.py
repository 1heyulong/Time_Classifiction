...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output = model(x)
            loss = loss_fn(output, y.unsqueeze(1).float())
            # criterion(output, y.unsqueeze(1).float())
            loss.backward()
            optimizer.step()
    ...
    with open(results_file, "a", encoding="utf-8") as f:
        f.write(f"ICB:{args.ICB},ASB:{args.ASB},Ir优化器:{args.start},描述:{args.name},每次实验之前, 需要自己想想怎么描述实验结果:\n")
