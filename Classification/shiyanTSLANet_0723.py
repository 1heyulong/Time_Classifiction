...
class TSLANet(nn.Module):
    def __init__(self, input_dim=1, seq_len=1034, embed_dim=64, num_classes=2, use_anomaly_head=True):
        super().__init__()
        ...
        self.asb = nn.Sequential(
            nn.Conv1d(embed_dim, embed_dim, kernel_size=3, padding=1, groups=1),
            nn.BatchNorm1d(embed_dim),
            nn.ReLU()
        )

        # Inception-style Convolution Block (ICB)
        ...
        if self.use_anomaly_head:
            self.anomaly_head = nn.Sequential(
                nn.Conv1d(embed_dim, 1, kernel_size=1),  # 每个时间点一个值
                nn.Sigmoid()
            )
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            anomaly_map: [B, L] if use_anomaly_head else None
        """
        x = self.patch_embed(x)     # [B, C, L]
        x = self.asb(x)
        x = self.icb(x)

        # 主任务：分类
        pooled = self.global_pool(x).squeeze(-1)   # [B, C]
        cls_pred = self.cls_head(pooled)           # [B, num_classes]

        # 辅助任务：异常段定位（每个位置预测一个“异常注意力”值）
        if self.use_anomaly_head:
            anomaly_map = self.anomaly_head(x).squeeze(1)  # [B, L]
            return cls_pred, anomaly_map
        else:
            return cls_pred, None

class model_pretraining(L.LightningModule):
    def __init__(self):
        super().__init__()
        self.save_hyperparameters({
    "anomaly_loss_weight": 0.1,  # ⬅️ 辅助损失的权重，可调
    "use_anomaly_head": True
})
        self.model = TSLANet()

    def forward(self, x):
        cls_pred, anomaly_map = self.model(x)
        return cls_pred, anomaly_map
...
class model_training(L.LightningModule):
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        preds,_ = self.model.forward(data)
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch  # y: [B]
        cls_pred, anomaly_map = self(x)

        # 主任务损失
        cls_loss = self.criterion(cls_pred, y)

        # 辅助任务损失（如果模型开启辅助头）
        aux_loss = 0.0
        if self.hparams.use_anomaly_head and anomaly_map is not None:
            # 用于引导注意力关注异常段 —— 无监督伪标签：只关注整体异常标签的样本
            pseudo_mask = (y == 1).float().unsqueeze(1)   # [B, 1]
            anomaly_loss = F.binary_cross_entropy(anomaly_map.mean(dim=1, keepdim=True), pseudo_mask)
            aux_loss = anomaly_loss * self.hparams.anomaly_loss_weight
