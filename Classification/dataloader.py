...
def get_datasets(DATASET_PATH, args):
    train_file = torch.load(os.path.join(DATASET_PATH, f"train.pt"), weights_only=True)
    seq_len = train_file["samples"].shape[-1]
    required_padding = calculate_padding(seq_len, args.patch_size)

    val_file = torch.load(os.path.join(DATASET_PATH, f"val.pt"), weights_only=True)
    test_file = torch.load(os.path.join(DATASET_PATH, f"test.pt"), weights_only=True)
