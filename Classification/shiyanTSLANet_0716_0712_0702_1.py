...
def extract_stat_features(x, window_size=64, stride=32):
    """
    输入: x of shape [B, C, T]
    输出: stat_feat of shape [B, C_stat, T_stat]
    """
    B, C, T = x.shape
    features = []
    for i in range(0, T - window_size + 1, stride):
        segment = x[:, :, i:i+window_size]  # [B, C, W]
        mean = segment.mean(dim=2, keepdim=True)  # [B, C, 1]
        std = segment.std(dim=2, keepdim=True)
        maxv = segment.max(dim=2, keepdim=True)[0]
        minv = segment.min(dim=2, keepdim=True)[0]
        feat = torch.cat([mean, std, maxv, minv], dim=1)  # [B, 4*C, 1]
        features.append(feat)
    features = torch.cat(features, dim=2)  # [B, 4*C, num_windows]
    return features
...
class TSLANet(L.LightningModule):
    ...
    def _augment_with_stat_feat(self, x):
        stat_feat = extract_stat_features(x, window_size=64, stride=32)  # [B, 4C, T_stat]
        stat_feat = F.interpolate(stat_feat, size=x.shape[-1], mode='linear')  # 对齐 T
        x = torch.cat([x, stat_feat], dim=1)  # Channel concat
        return x
    ...
    def forward(self, x, return_feat=False):
        if args.use_stat_feat:
            x = self._augment_with_stat_feat(x)
...
class model_training(L.LightningModule):
    def __init__(self):
        ...
        # self.criterion = LabelSmoothingCrossEntropy(weight=args.class_weights)
    ...
    def configure_optimizers(self):
        optimizer = optim.AdamW(self.parameters(), lr=args.train_lr, weight_decay=1e-3)
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        if mode == "train":
            ...
            loss_cl = nt_xent_loss(feat1, feat2, temperature=0.5)

            total_loss = loss_cls + args.loss_rate * loss_cl  # 平衡系数可调
...
def train_model(pretrained_model_path):
    ...
    trainer = L.Trainer(
        ...
        callbacks=[
            ...
            EarlyStopping(
                monitor='val_f1',
                patience=500,  # 如果验证集损失在10个epoch内没有改善
                mode='max',  # 监控最小值
            ),
    ...
    get_clf_report(model, val_loader, CHECKPOINT_PATH, args.class_names)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_smoothing', type=float, default=0.1, help='Value for label smoothing.')
    # TSLANet components:
    ...
    # 3. argparse 添加开关参数
    parser.add_argument('--use_stat_feat', type=str2bool, default=False, help='是否使用滑窗统计特征增强')
    ...
    train_labels = train_loader.dataset.y_data
    ...
    # --- 调试代码开始 ---
    print("\n--- Debugging compute_class_weight ---")
    print(f"Value of 'unique_labels' passed to 'classes': {unique_labels}")
    print(f"Data type of 'unique_labels': {unique_labels.dtype}")
    
    print(f"Type of 'train_labels' passed to 'y': {type(train_labels)}")
    print(f"Shape of 'train_labels': {train_labels.shape if hasattr(train_labels, 'shape') else 'N/A'}")
    print(f"Data type of 'train_labels' elements: {train_labels.dtype if hasattr(train_labels, 'dtype') else 'No dtype'}")

    # 为了安全起见，再次从 y 中提取唯一值进行比较
    y_unique_check = np.unique(train_labels)
    print(f"Unique labels found in 'y' right before the call: {y_unique_check}")
    
    # 检查我们准备传入的 'classes' 和从 'y' 中实际找到的是否完全一致
    are_equal = np.array_equal(unique_labels, y_unique_check)
    print(f"Are 'classes' and unique labels from 'y' identical? -> {are_equal}")
    if not are_equal:
        print("!!! WARNING: Mismatch detected between 'unique_labels' and what's actually in 'train_labels'!!!")
    ...
    args.class_weights = torch.tensor(class_weights, dtype=torch.float)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        filename='0712_0702_train_{epoch}_{val_f1:.4f}', # 在文件名中体现F1分数
        monitor='val_f1',
        mode='max'
    )
