...
class ICB(L.LightningModule):
    def __init__(self, in_features, hidden_features, drop=0.):
        super().__init__()
        self.conv1 = spectral_norm(nn.Conv1d(in_features, hidden_features, 1))
        self.conv2 = spectral_norm(nn.Conv1d(in_features, hidden_features, 3, 1, 1))
        self.conv3 = spectral_norm(nn.Conv1d(hidden_features, in_features, 1))
...
class TSLANet_layer(L.LightningModule):
    def __init__(self, dim, mlp_ratio=3., drop=0.3, drop_path=0.1):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.asb = Adaptive_Spectral_Block(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = nn.LayerNorm(dim)
        self.dropout_post = nn.Dropout(p=drop)
